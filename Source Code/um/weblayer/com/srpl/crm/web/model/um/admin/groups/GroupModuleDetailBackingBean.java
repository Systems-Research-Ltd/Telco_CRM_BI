	...
	public String actionListener() {
		...
		case WebConstants.ACTION_SAVE:
			...
			try {
				...
				this.addMessage(getProperty("group.successfully.created"));
			} catch (Exception e) {
				this.addError(getProperty("group.creation.fail"));
			}
			this.detailView();
			reset();
			setViewAction();
		...
		case WebConstants.ACTION_UPDATE:
			...
			try {
				...
				this.addMessage(getProperty("group.successfully.updated"));
			} catch (Exception e) {
				this.addError(getProperty("group.update.fail"));
			}
			reset();
			this.detailView();
			break;
		...
		case WebConstants.ACTION_DELETE_CONFIRMED:
			try {
				...
				this.addMessage(getProperty("group.successfully.deleted"));
			} catch (Exception e) {
				this.addError(getProperty("group.deletion.fail"));
			}
	...
	*/
	...
	public List<AjaxListStructure> getList() {
		...
		if(myList.size() == 0){
			...
			e = new AjaxListStructure();
			e.setId(0L);
			e.setLabel(getProperty("no.group.found"));
