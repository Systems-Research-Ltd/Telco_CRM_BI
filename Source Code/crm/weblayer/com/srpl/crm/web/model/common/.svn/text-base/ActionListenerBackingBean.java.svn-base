...
//		BeanFactory.getInstance().getSampleBackingBean().add(action);
		if (WebConstants.TAB_MARKETING.equals(action)) {
			...
			BeanFactory.getInstance().getSessionBean().setSalesModule_selectedOrder(null);
		...
		} else if (WebConstants.TAB_IMPORT.equals(action)) {
			return WebConstants.TAB_IMPORT;
...
//		BeanFactory.getInstance().getSampleBackingBean().add(action);
		if (WebConstants.MENU_1.equals(action)) {
			...
			FacesContext.getCurrentInstance().getExternalContext().getSessionMap().remove("import");
		...
		else if (WebConstants.MENU_SETUP.equals(action)) {
			return WebConstants.MENU_SETUP;
		}
		else if (WebConstants.QUERY_TYPE.equals(action)) {
			System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> i am about to return QUERY_TYPE");
			return WebConstants.QUERY_TYPE;
		}
		return "failure";
	}
